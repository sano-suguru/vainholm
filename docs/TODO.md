...
- [ ] 緑プレミアム（所持効果）の型定義追加 (`combat/types.ts`)
- [ ] 緑プレミアム効果の実装（HP+、視界+、罠感知、忍び足）
- [ ] 武器プレミアム調整：`かず+N`（攻撃回数）の追加
- [ ] 武器プレミアム調整：`貫通`（敵貫通攻撃）の追加
- [ ] ドロップ時の光色表示（白/青/緑/青緑/金）
- [ ] 防具システムの基盤実装（防具タイプ、スロット）
- [ ] 防具プレミアム（青）の実装（物理耐性、属性耐性、反射、棘）
- [ ] 強化の巻物の実装（希少アイテム、プレミアム枠追加）

### Phase B: 崩壊システム

片道勇者要素の核心。時間制約による緊張感を導入。

- [ ] 崩壊状態の型定義追加 (`dungeon/types.ts`)
- [ ] dungeonStoreに崩壊進行ロジック追加
- [ ] 崩壊開始トリガー（200ターン後）
- [ ] 崩壊速度設定（50ターン/1F）
- [ ] 崩壊したフロアへのアクセス制限
- [ ] 崩壊警告のHUD表示
- [ ] 崩壊に巻き込まれた場合のゲームオーバー処理
- [ ] カジュアルモード（崩壊なし）のオプション追加
